...
import { CryptoCalcResult, CryptoSimulationResult, simulateCryptoAtPrice } from '../utils/calculator';
...
export const CryptoSimulation: React.FC<CryptoSimulationProps> = ({ calcResult }) => {
  const [targetPrice, setTargetPrice] = useState<number>(0);
  const [simResult, setSimResult] = useState<CryptoSimulationResult | null>(null);
  ...
  useEffect(() => {
    if (calcResult && targetPrice > 0) {
      setSimResult(simulateCryptoAtPrice(calcResult, targetPrice));
    }
  ...
  const minPrice = calcResult.averagePriceExact * 0.5;
  const maxPrice = calcResult.averagePriceExact * 2;
    ...
    <div className="card animate-fade-in-up" id="simulation-card">
      ...
      <div className="simulation-slider-container">
        <div className="slider-header">
          <span>Target Jual: <strong className="gold" id="sim-target-price">{formatRupiah(targetPrice)}</strong></span>
        ...
        <div className="slider-labels">
          ...
          <span>{formatRupiah(calcResult.averagePriceExact)} (Avg)</span>
